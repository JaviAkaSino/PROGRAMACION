...
public final class Baraja {
    ...
    //Cada baraja contiene las 52 cartas diferentes
    ...
    public void llenarBaraja() {
        ...
        for (int i = 0; i < 4; i++) { //Número de palos
            for (int j = 0; j < 13; j++) { //Número de valores

                this.baraja.add(new Carta(Valores.values()[j], Palos.values()[i]));
            }
        }
    }
    ...
    public Carta cartaPorNumero(int numero) {
        ...
        for (Carta carta : this.baraja) { //Recorre la baraja
            ...
            if (carta.getValor().equals(Valores.values()[numero - 1])) { //Si la carta tiene el valor dado
